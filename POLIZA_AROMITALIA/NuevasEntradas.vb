...
Public Class NuevasEntradas
    Public Shared Sub Entradas()
        Using ConnEntrada As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
            Dim adaEntrada As New SqlDataAdapter("SELECT CVE_FOLIO FROM INSOFTEC_MINVE_IN WHERE CAMPLIB1 IS NULL AND CAMPLIB3 IS NULL", ConnEntrada)
                ...
                Using ConnEntrada1 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                    ...
                    Do While (y <= x)
                        ...
                        Using ConAux As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                            ConAux.Open()
                            Dim cmdAux As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('Dr',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'" & con1 + 1 & "','" & PERIODO & "','" & EJERCICIO & "','" & CAMPLIB8 & "000000000003" & "','" & FECH & "','" & REFERMIN & " -" & CVEART & "','D',' " & Math.Round(CANTMIN * COSTOMIN, 2) & "','0','1','0','" & con1 + 1 & "','0','0');"), ConAux)
                        ...
                        Using ConAux As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                            ConAux.Open()
                            Dim cmdAux As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('Dr',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'" & con2 + 1 & "','" & PERIODO & "','" & EJERCICIO & "','" & CAMPLIB9 & "000000000002" & "','" & FECH & "','COSTO -" & CVEART & "','H','" & Math.Round(CANTMIN * COSTOMIN, 2) & "','0','1','0','" & con2 + 1 & "','0','0');"), ConAux)
                            cmdAux.ExecuteNonQuery()
                            ConAux.Close()
                        End Using
                        totpartida = 0
