...
Public Class CuentaSalida
    Public Shared CAMPLIB3S As String = ""
    Public Shared CAMPLIB4S As String = ""
    Public Shared Sub AccountOut()
        Using ConnEntrada As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
            Dim adaEntrada As New SqlDataAdapter("SELECT CVE_FOLIO FROM INSOFTEC_MINVE_OUT WHERE CAMPLIB1 IS NULL", ConnEntrada)
            ...
            Do While (j <= i)
                ...
                Using ConnEntrada1 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                    ...
                    Do While (y <= x)
                        CVEART = dtEntrada1.AsEnumerable().ElementAtOrDefault(y).Item(0).ToString
                ...
                Using ConInveClib As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                    ConInveClib.Open()
                    Dim adaInveClib As SqlDataReader = New SqlCommand("SELECT CAMPLIB3,CAMPLIB4 FROM INSOFTEC_MINVE_OUT WHERE CVE_FOLIO = '" & CVEFOLIO & "'", ConInveClib).ExecuteReader
                    adaInveClib.Read()
                    CAMPLIB3S = adaInveClib.Item(0).ToString
                    CAMPLIB4S = adaInveClib.Item(1).ToString
                    ConInveClib.Close()
                End Using

                Using ConnMinveInsof As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                    ConnMinveInsof.Open()
                    If CS > 0 AndAlso CAMPLIB3S = "" AndAlso CAMPLIB4S = "" Then
                        EME()
                        Dim cmdConnMinveinsof As New SqlCommand(("UPDATE INSOFTEC_MINVE_OUT SET CAMPLIB3='S/CTA', CAMPLIB4='ENVIADO' WHERE CVE_FOLIO='" & CVEFOLIO & "'"), ConnMinveInsof)
                        cmdConnMinveinsof.ExecuteNonQuery()
                    End If
                    If CS = 0 Then
                        Dim cmdConnMinveinsofC As New SqlCommand(("UPDATE INSOFTEC_MINVE_OUT SET CAMPLIB3=NULL WHERE CVE_FOLIO='" & CVEFOLIO & "'"), ConnMinveInsof)
