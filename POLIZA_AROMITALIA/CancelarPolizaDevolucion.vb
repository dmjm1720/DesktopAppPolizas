...
Public Class CancelarPolizaDevolucion
    Public Shared Sub FacturaPolizaCanceldaDevolucion()
        Using cn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
            Dim ada As New SqlDataAdapter("SELECT CVE_DOC, FECHA_DOC FROM  " & FACTD & " INNER JOIN INSOFTEC_FACTD_CLIB01 ON " & FACTD & ".CVE_DOC = INSOFTEC_FACTD_CLIB01.CLAVE_DOC WHERE (INSOFTEC_FACTD_CLIB01.CAMPLIB1 >'" & TIPOFACTD & "' AND INSOFTEC_FACTD_CLIB01.CAMPLIB2 IS NULL) AND (" & FACTD & ".STATUS = 'C')", cn)
            ...
            Try
                Do While (i <= j)
                    ...
                    Using cn1 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                        cn1.Open()
                        Dim cmd1 As New SqlCommand(("UPDATE " & AUXILIAR & " set MONTOMOV = 0 WHERE TIPO_POLI='" & TIPOFACTD & "' AND CONCEP_PO LIKE '%" + CVE_DOC + "%'"), cn1)
                        cmd1.ExecuteNonQuery()
                        cn1.Close()
                        Using cn2 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                            cn2.Open()
                            Dim command6611 As New SqlCommand(("UPDATE INSOFTEC_FACTD_CLIB01 SET CAMPLIB1 ='CANCELADO', CAMPLIB3='PROCESO 4' WHERE CLAVE_DOC= '" + CVE_DOC + "'"), cn2)
                        ...
                        Using cn3 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                            cn3.Open()
                            Dim adapter3 As SqlDataReader = New SqlCommand("SELECT CONCEP_PO FROM " & POLIZA & "  WHERE TIPO_POLI='" & TIPOFACTD & "' AND CONCEP_PO LIKE '%" + CVE_DOC + "%'", cn3).ExecuteReader
                            adapter3.Read()
                        ...
                        Using cn4 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                            cn4.Open()
                            'ACTUALIZA EL FOLIO
                            Dim command6611 As New SqlCommand(("UPDATE " & POLIZA & " SET CONCEP_PO='" & DATO & "  **CANCELADO**" & "', CONTABILIZ='N' WHERE TIPO_POLI='" & TIPOFACTD & "' AND CONCEP_PO  = '" & DATO & "'"), cn4)
                            command6611.ExecuteNonQuery()
                            cn4.Close()
