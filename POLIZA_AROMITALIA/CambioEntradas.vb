...
Public Class CambioEntradas
    Public Shared FOLIODR As String = ""
    Public Shared CERRADOENTRADAS As String = ""
    Public Shared Sub NuevoCambioEntrada()
        Using ConnEntrada As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
            Dim adaEntrada As New SqlDataAdapter("SELECT CVE_FOLIO, CAMPLIB1 FROM INSOFTEC_MINVE_IN WHERE CAMPLIB10=1 AND CAMPLIB1<>'NULL'", ConnEntrada)
            ...
            Do While (j <= i)
                ...
                FOLIODR = dtEntrada.AsEnumerable().ElementAtOrDefault(j).Item(1)
                Dim cut As String = FOLIODR.Substring(2)
                Using ConnEntrada1 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                    ...
                    Do While (y <= x)
                        ...
                        Using ConnPeriod As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                            ...
                            AdaPeriod.Read()
                            CERRADOENTRADAS = AdaPeriod.Item(2)
                            ConnPeriod.Close()
                        End Using
                        ...
                        Using ConAux As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                            ConAux.Open()
                            If CERRADOENTRADAS = 0 Then '******************************PARA VALIDAR SI EL PERIODO ESTA ABIERTO
                                Dim cmdAux As New SqlCommand("UPDATE " & AUXILIAR & " SET MONTOMOV = '" & CANTMIN * COSTOMIN & "' WHERE NUM_POLIZ LIKE '%" & cut & "%' AND PERIODO='" & PERIODO & "' AND TIPO_POLI='Dr' AND CONCEP_PO LIKE '%" & REFERMIN & " -" & CVEART & "%' AND NUM_CTA ='" & CAMPLIB8 & "000000000003" & "' AND NUM_PART='" & contarPartidas & "'", ConAux)
                        ...
                        Using ConAux As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                            ConAux.Open()
                            If CERRADOENTRADAS = 0 Then '******************************PARA VALIDAR SI EL PERIODO ESTA ABIERTO
                                Dim cmdAux As New SqlCommand("UPDATE " & AUXILIAR & " SET MONTOMOV = '" & CANTMIN * COSTOMIN & "' WHERE NUM_POLIZ LIKE '%" & cut & "%' AND PERIODO='" & PERIODO & "' AND TIPO_POLI='Dr' AND CONCEP_PO LIKE '%" & CVEART & "%' AND NUM_CTA ='" & CAMPLIB9 & "000000000002" & "' AND NUM_PART='" & contarPartidas + 1 & "'", ConAux)
                                cmdAux.ExecuteNonQuery()
                            End If
                            ConAux.Close()
                        End Using
                ...
                Using ConnMinveInsof As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                    ConnMinveInsof.Open()
                    Dim cmdConnMinveinsof As New SqlCommand("UPDATE INSOFTEC_MINVE_IN SET CAMPLIB10=NULL WHERE CVE_FOLIO='" & CVEFOLIO & "'", ConnMinveInsof)
                ...
                Using ConnPoliza As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    ConnPoliza.Open()
                    If CERRADOENTRADAS = 0 Then '**************PARA VALIDAR SI ESTA CERRADO EL PERIODO
                        Dim cmdPol As New SqlCommand("UPDATE " & POLIZA & " SET CONTABILIZ='N' WHERE CONCEP_PO ='" & "ENTRADA " & REFERMIN + "  " & FECHAELABMIN.ToString("dd-MMMM-yyyy").ToUpper & "' AND NUM_POLIZ LIKE '%" & cut & "%' AND PERIODO=' " & PERIODO & "' AND TIPO_POLI='Dr'", ConnPoliza)
