...
Public Class ProcesarCompra
    Public Shared Sub ProcesarNuevaCompra()
        Dim num As Integer = 0
        Dim LEVEL1 As String = ""
        Dim LEVEL2 As String = ""
        Dim LEVEL3 As String = ""
        Dim CADENA As String = ""
        Dim ContadorIndirecto As Integer = 0
        Dim ContarCuenta As Integer = 0
        ...
        Using connection As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
            ...
            Do While (i <= num6)
                ...
                TOT_IND = dataTable.AsEnumerable().ElementAtOrDefault(i).Item(34).ToString

                'IMPORTE = IMPORTE + TOT_IND
                ...
                Using INSERT_FOLIO As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    ...
                    INSERT_FOLIO.Open()
                    'ACTUALIZA EL FOLIO
                    Dim command661 As New SqlCommand(("UPDATE " & FOLIOS & " SET FOLIO" & PERIODO1 & " ='" & contador & "' WHERE EJERCICIO ='" & EJERCICIO & "' AND TIPPOL='" & TIPOCOMPC & "'"), INSERT_FOLIO)
                ...
                Using BUSCAPARTIDAS As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                    Dim ada1 As New SqlDataAdapter("SELECT NUM_PAR,CVE_ART,CANT,PREC,COST,IMPU4,IMP1APLA,IMP2APLA,IMP3APLA,ACT_INV,NUM_ALM,TIP_CAM,UNI_VENTA,TIPO_PROD,CVE_OBS,REG_SERIE,E_LTPD,TIPO_ELEM,NUM_MOV,TOT_PARTIDA,MINDIRECTO FROM " & PAR_COMPC & " WHERE CVE_DOC='" & CVE_DOC & "'", BUSCAPARTIDAS)
                    ...
                    Do While (j <= num07)
                        ...
                        MINDIRECTO = data1.AsEnumerable().ElementAtOrDefault(j).Item(20)
                        ...
                        Using connection2225581 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                            ...
                            End If
                            If MYACCOUNT = "113001001000000000003" Or MYACCOUNT = "113001002000000000003" Then
                                TOT_PARTIDA = TOT_PARTIDA + MINDIRECTO
                            End If
                            ...
                            Using INSERT_PARTIDAS As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                                ...
                                If v = True Then
                                    Dim command6 As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'" & K & "','" & PERIODO & "','" & EJERCICIO & "','" & MYACCOUNT & "','" & FECH & "','" + CVE_DOC + "  " + CVE_ART + "  " + CONCEPTO2 + "','D','" & Math.Round(TOT_PARTIDA, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & NUM_PAR1 & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDAS) 'LIN_PROD2                           
                                    command6.ExecuteNonQuery()
                                Else
                                    DEPTO = "0"
                                    Dim command6 As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'" & K & "','" & PERIODO & "','" & EJERCICIO & "','" & MYACCOUNT & "','" & FECH & "','" + CVE_DOC + "  " + CVE_ART + "  " + MIDESCR + "','D','" & Math.Round(TOT_PARTIDA, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & NUM_PAR1 - 1 & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDAS) 'PARTIDAS SIN DESCUENTOS
                                    command6.ExecuteNonQuery()
                                End If
                    ...
                    If TOT_IND > 0 Then
                        Using INSERT_TOTPART As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                            INSERT_TOTPART.Open()
                            Dim adapter50 As SqlDataReader = New SqlCommand("SELECT COUNT(NUM_PART)  FROM " & AUXILIAR & " WHERE TIPO_POLI='" & TIPOCOMPC & "' AND NUM_CTA LIKE '113001%' AND PERIODO=" & PERIODO & " AND NUM_POLIZ=" & contador & "", INSERT_TOTPART).ExecuteReader
                            Try
                                adapter50.Read()
                                ContarCuenta = adapter50.Item(0)
                        ...
                        Using INSERT_TOTPART As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                            INSERT_TOTPART.Open()
                            Dim adapter50 As SqlDataReader = New SqlCommand("SELECT TOP(1) NUM_CTA  FROM " & AUXILIAR & " WHERE TIPO_POLI='" & TIPOCOMPC & "' AND PERIODO=" & PERIODO & " AND NUM_CTA = '113001002000000000003' AND NUM_POLIZ=" & contador & "", INSERT_TOTPART).ExecuteReader
                            Try
                                adapter50.Read()
                                NuevaCuenta = adapter50.Item(0)
                            Catch ex As Exception
                                INSERT_TOTPART.Close()
                            End Try
                        End Using
                        If NuevaCuenta = "" Then
                            Using INSERT_TOTPART As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                                INSERT_TOTPART.Open()
                                Dim adapter50 As SqlDataReader = New SqlCommand("SELECT TOP(1) NUM_CTA  FROM " & AUXILIAR & " WHERE TIPO_POLI='" & TIPOCOMPC & "' AND PERIODO=" & PERIODO & " AND NUM_CTA = '113001001000000000003' AND NUM_POLIZ=" & contador & "", INSERT_TOTPART).ExecuteReader
                                Try
                                    adapter50.Read()
                                    NuevaCuenta = adapter50.Item(0)
                                Catch ex As Exception
                        ...
                        If ContarCuenta > 0 Then
                            Using INSERT_TOTPART As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                                INSERT_TOTPART.Open()
                                Dim adapter50 As SqlDataReader = New SqlCommand("SELECT count(NUM_PART)  FROM " & AUXILIAR & " WHERE TIPO_POLI='" & TIPOCOMPC & "' AND CONCEP_PO LIKE '%" & CVE_DOC & "%' AND NUM_POLIZ=" & contador & "", INSERT_TOTPART).ExecuteReader
                                adapter50.Read()
                                contar = adapter50.Item(0)
                                INSERT_TOTPART.Close()
                            End Using
                            Using TotIndi As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                                TotIndi.Open()
                                Dim cmdInsert As New SqlCommand("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'" & contar + 2 & "','" & PERIODO & "','" & EJERCICIO & "','" & NuevaCuenta & "','" & FECH & "','COMPRA " + CVE_DOC + "  " + CONCEPTO + "  " + CONCEPTO2 + "','H','" & Math.Round(TOT_IND, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & contar + 2 & "','" & ccostos & "','" & cuentacon & "');", TotIndi)
                    ...
                    Using INSERT_PARTIDAS As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                        ...
                        If v = True Then
                            Dim command6 As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'" & NUM_PAR & "','" & PERIODO & "','" & EJERCICIO & "','" & CUENTA_CONTABLE + "000000000003" & "','" & FECH & "','" + "COMPRA " & CVE_DOC + "    " + CONCEPTO + " " & "','H','" & Math.Round(IMPORTE, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & NUM_PAR + 1 & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDAS)
                            command6.ExecuteNonQuery()
                        Else
                            DEPTO = "0"
                            Dim command6 As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'" & NUM_PAR + 1 & "','" & PERIODO & "','" & EJERCICIO & "','" & CUENTA_CONTABLE + "000000000003" & "','" & FECH & "','" + "COMPRA " & CVE_DOC & "','H','" & Math.Round(IMPORTE, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & NUM_PAR & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDAS)
                            command6.ExecuteNonQuery()
                        End If
                        INSERT_PARTIDAS.Close()
                    End Using
                    ...
                    Using INSERT_PARTIDAS As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                        If IMP_TOT4 > 0 Then
                            ...
                            If v = True Then
                                Dim commandD As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'555','" & PERIODO & "','" & EJERCICIO & "','" & DEF1 & "','" & FECH & "','" + "COMPRA " & CVE_DOC & "','D','" & Math.Round(IMP_TOT4, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & NUM_PAR2 & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDAS) 'DEF1                           
                                commandD.ExecuteNonQuery()

                            Else
                                DEPTO = "0"
                                Dim commandD As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'555','" & PERIODO & "','" & EJERCICIO & "','" & CUENTA_COI & "','" & FECH & "','" + "COMPRA " & CVE_DOC & "','D','" & Math.Round(IMP_TOT4, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & NUM_PAR2 + 1 & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDAS)
                                commandD.ExecuteNonQuery()
                            End If
                    ...
                    Using INSERT_CAMP As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                        INSERT_CAMP.Open()
                        Dim command666 As New SqlCommand(("UPDATE " & AUXILIAR & " SET NUM_PART='" & MIPAR3 & "', ORDEN='" & MIPAR3 & "' WHERE NUM_PART =555 AND TIPO_POLI='" & TIPOCOMPC & "' AND CONCEP_PO LIKE '%" & CVE_DOC & "%'"), INSERT_CAMP)
                    ...
                    Using INSERT_PARTIDASs As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                        If IMP_TOT1 > 0 Then
                            ...
                            If x = x Then
                                CONCEPTO = Replace(CONCEPTO, "'", "")
                                Dim commandDs As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('    ' + LTRIM(RTRIM(" & contador & ")), 5),'666','" & PERIODO & "','" & EJERCICIO & "','" & DEF2 & "','" & FECH & "','" + "COMPRA " & CVE_DOC & "','H','" & Math.Round(IMP_TOT1, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & NUM_PAR2 + 1 & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDASs) 'DEF1                         
                                commandDs.ExecuteNonQuery()
                            Else
                                Dim commandDs As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & j & ")), 5),'666','" & PERIODO & "','" & EJERCICIO & "','" & CUENTA_COI & "','" & FECH & "','" + "COMPRA " & CVE_DOC + "  " + CVE_ART + "  " + CONCEPTO3 + "','H','" & Math.Round(IMP_TOT1, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & NUM_PAR2 & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDASs)
                                commandDs.ExecuteNonQuery()
                            End If
                    ...
                    Using INSERT_CAMP As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                        INSERT_CAMP.Open()
                        Dim command666 As New SqlCommand(("UPDATE " & AUXILIAR & " SET NUM_PART='" & MIPAR4 & "', ORDEN='" & MIPAR4 & "' WHERE NUM_PART =666 AND TIPO_POLI='" & TIPOCOMPC & "' AND CONCEP_PO LIKE '%" & CVE_DOC & "%'"), INSERT_CAMP)
                        command666.ExecuteNonQuery()
                        INSERT_CAMP.Close()
                    End Using
                ...
                Using INSERT_FOLIO As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    INSERT_FOLIO.Open()
                    Dim command661 As New SqlCommand(("UPDATE " & AUXILIAR & " set NUM_PART=1 WHERE NUM_PART=0 AND TIPO_POLI='" & TIPOCOMPC & "'"), INSERT_FOLIO)
                    command661.ExecuteNonQuery()
                    INSERT_FOLIO.Close()
                End Using
                ...
                Using INSERT_PARTIDASs As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    ...
                    If IMP_RET > 0 Then
                        INSERT_PARTIDASs.Open()
                        Dim commandDs As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'333','" & PERIODO & "','" & EJERCICIO & "','200013000000000000002','" & FECH & "','" + "IMPUESTO RETENIDO " & CVE_DOC + " ','D','" & Math.Round(IMP_TOT2, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & MIPAR1 + 1 & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDASs)
                        commandDs.ExecuteNonQuery()
                        INSERT_PARTIDASs.Close()
                    End If
                End Using
                ...
                Using INSERT_CAMP As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    INSERT_CAMP.Open()
                    Dim command666 As New SqlCommand(("UPDATE " & AUXILIAR & " SET NUM_PART='" & MIPAR1 & "', ORDEN='" & MIPAR1 & "' WHERE NUM_PART =333 AND TIPO_POLI='" & TIPOCOMPC & "' AND CONCEP_PO LIKE '%" & CVE_DOC & "%'"), INSERT_CAMP)
                    command666.ExecuteNonQuery()
                    INSERT_CAMP.Close()
                End Using
                ...
                Using INSERT_PARTIDASs As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    ...
                    If IMP_RET2 > 0 Then
                        INSERT_PARTIDASs.Open()
                        Dim commandDs As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'567','" & PERIODO & "','" & EJERCICIO & "','200013000000000000002','" & FECH & "','" + "IMPUESTOS RETENIDOS " & CVE_DOC + " ','D','" & Math.Round(IMP_TOT3, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & MIPAR1 + 1 & "','" & ccostos & "','" & cuentacon & "');"), INSERT_PARTIDASs)
                        commandDs.ExecuteNonQuery()
                        INSERT_PARTIDASs.Close()
                    End If
                End Using
                ...
                Using INSERT_CAMP As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    INSERT_CAMP.Open()
                    Dim command666 As New SqlCommand(("UPDATE " & AUXILIAR & " SET NUM_PART='" & MIPAR1 & "', ORDEN='" & MIPAR1 & "' WHERE NUM_PART =567 AND TIPO_POLI='" & TIPOCOMPC & "' AND CONCEP_PO LIKE '%" & CVE_DOC & "%'"), INSERT_CAMP)
                    command666.ExecuteNonQuery()
                    INSERT_CAMP.Close()
                End Using
                ...
                Using INSERT_CAMP As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                    ...
                    INSERT_CAMP.Open()
                    Dim command666 As New SqlCommand(("UPDATE INSOFTEC_COMPC_CLIB01 SET CAMPLIB1='" & TIPOCOMPC & contador & "' WHERE CLAVE_DOC='" & CVE_DOC & "'"), INSERT_CAMP)
                    command666.ExecuteNonQuery()
                    INSERT_CAMP.Close()
                End Using
                ...
                Try
                    Using connection22 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                        connection22.Open()
                        Dim adapter2231 As SqlDataReader = New SqlCommand("SELECT SUM(TOT_PARTIDA) FROM " & PAR_COMPC & " WHERE CVE_DOC ='" & CVE_DOC & "' AND IMPU4=0", connection22).ExecuteReader
                        adapter2231.Read()
                ...
                Try
                    Using connection22 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                        connection22.Open()
                        Dim adapter2231 As SqlDataReader = New SqlCommand("SELECT SUM(TOT_PARTIDA - (TOT_PARTIDA * (DESC1 / 100)) - ((TOT_PARTIDA - (TOT_PARTIDA * (DESC1 / 100))) * (DESC2 / 100))) FROM " & PAR_COMPC & " WHERE CVE_DOC ='" & CVE_DOC & "' AND IMPU4=0", connection22).ExecuteReader
                        adapter2231.Read()
                ...
                Using INSERT_PARTIDASs As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    If DESCCERO > 0 Then
                        INSERT_PARTIDASs.Open()
                        Dim commandDs As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES (" & PAR_COMPC & ",RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'333','" & PERIODO & "','" & EJERCICIO & "','410002000000000000002','" & FECH & "','" + "DESCUENTO " & CVE_DOC + " ','D','" & Math.Round(DESCCERO, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & MIPAR1 + 1 & "','" & ccostos & "'," & cuentacon & ");"), INSERT_PARTIDASs)
                        commandDs.ExecuteNonQuery()
                        INSERT_PARTIDASs.Close()
                    End If
                End Using
                ...
                Try
                    Using connection22 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                        connection22.Open()
                        Dim adapter2231 As SqlDataReader = New SqlCommand("SELECT SUM(TOT_PARTIDA) FROM " & PAR_COMPC & " WHERE CVE_DOC ='" & CVE_DOC & "' AND IMPU4=16", connection22).ExecuteReader
                        adapter2231.Read()
                ...
                Try
                    Using connection22 As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                        connection22.Open()
                        Dim adapter2231 As SqlDataReader = New SqlCommand("SELECT SUM(TOT_PARTIDA - (TOT_PARTIDA * (DESC1 / 100)) - ((TOT_PARTIDA - (TOT_PARTIDA * (DESC1 / 100))) * (DESC2 / 100))) FROM " & PAR_COMPC & " WHERE CVE_DOC ='" & CVE_DOC & "' AND IMPU4=16", connection22).ExecuteReader
                        adapter2231.Read()
                ...
                Using INSERT_PARTIDASs As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    If DESC16 > 0 Then
                        INSERT_PARTIDASs.Open()
                        Dim commandDs As New SqlCommand(("INSERT INTO " & AUXILIAR & " (TIPO_POLI, NUM_POLIZ, NUM_PART, PERIODO, EJERCICIO, NUM_CTA, FECHA_POL, CONCEP_PO, DEBE_HABER, MONTOMOV, NUMDEPTO,TIPCAMBIO,CONTRAPAR,ORDEN, CCOSTOS,CGRUPOS) VALUES ('" & TIPOCOMPC & "',RIGHT('     ' + LTRIM(RTRIM(" & contador & ")), 5),'444','" & PERIODO & "','" & EJERCICIO & "','410001000000000000002','" & FECH & "','" + "DESCUENTO " & CVE_DOC + " ','D','" & Math.Round(DESC16, 2) & "','" & DEPTO & "','" & TIPCAMB & "','0','" & MIPAR2 + 1 & "','" & ccostos & "'," & cuentacon & ");"), INSERT_PARTIDASs)
                        commandDs.ExecuteNonQuery()
                        INSERT_PARTIDASs.Close()
                    End If
                End Using
                ...
                Using INSERT_CAMP As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("COI").ToString)
                    INSERT_CAMP.Open()
                    Dim command666 As New SqlCommand(("UPDATE " & AUXILIAR & " SET NUM_PART='" & MIPAR2 & "', ORDEN='" & MIPAR2 & "' WHERE NUM_PART =444 AND TIPO_POLI='" & TIPOCOMPC & "' AND CONCEP_PO LIKE '%" & CVE_DOC & "%'"), INSERT_CAMP)
                    command666.ExecuteNonQuery()
                    INSERT_CAMP.Close()
                End Using
                ...
                Using conectioncdfi As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings.Item("SAE").ToString)
                    conectioncdfi.Open()
                    Dim adaptercdfi As SqlDataReader = New SqlCommand("SELECT FECHA_TIMBRADO, ID_TIMBRADO FROM " & CFDII & " WHERE CVE_DOC = '" & CVE_DOC & "' AND RFC_EMISOR = '" & RFCPROVEEDOR & "'", conectioncdfi).ExecuteReader
                    adaptercdfi.Read()
